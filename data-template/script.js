...
function setButtonStyle(button, quantity) {
  if (quantity == 0) {
    button.innerHTML = '&#128197;';
...
function setQuantity(id, quantity) {
  ...
  // get popup button
  var popup_button = row.getElementsByTagName('button')[0];

  setButtonStyle(popup_button, quantity);

  timers[id].quantity = quantity;
}
...
function showTimer(id, timer) {
  ...
  // enabled checkbox
  var enabled_td = document.createElement('td');
  var enabled_checkbox = document.createElement('input');
  enabled_checkbox.type = 'checkbox';
  enabled_checkbox.checked = timer.enabled;
  enabled_checkbox.onclick = function () {
    setEnabled(id, enabled_checkbox.checked);
  }
  enabled_td.appendChild(enabled_checkbox);
  row.appendChild(enabled_td);

  // popup button
  var popup_td = document.createElement('td');
  var popup_button = document.createElement('button');

  setButtonStyle(popup_button, timer.quantity);

  popup_button.className = 'popup-button';
  popup_button.onclick = function () {
    showWeekdayConfig(id);
  }
  popup_td.appendChild(popup_button);
  row.appendChild(popup_td);

  // name input
  var name_td = document.createElement('td');
  var name_input = document.createElement('input');
  name_input.type = 'text';
  name_input.value = timer.name;
  name_input.className = 'name-input';
  name_input.onchange = function () {
    setName(id, name_input.value);
  }
  ...
  time_input.value = timer.time;
  time_input.className = 'time-input';
  time_input.onchange = function () {
    setTime(id, time_input.value);
  }
  time_td.appendChild(time_input);
  ...
  // feed quantity input
  document.getElementById('timers-body').appendChild(row);
}
...
function showLogs() {
  ...
  clear_button.innerHTML = 'Delete logs';
  clear_button.className = 'clear-button';
  clear_button.onclick = function () {
    if (window.confirm("Are you sure you want to physically delete the logs?")) {
...
function showWeekdayConfig(id) {
  ...
  feed_input.onchange = function () {
    setQuantity(id, feed_input.value);
  }
  feed_container.appendChild(feed_label);
  feed_container.appendChild(feed_input);

  var default_quantity = document.getElementById('feed').value;

  feeding_info = document.createElement('div');
  feeding_info.className = 'feeding-info';
  feeding_info.innerHTML = '*If the feed quantity is set to 0, the default quantity will be used (currently ' + default_quantity + 'g)';
